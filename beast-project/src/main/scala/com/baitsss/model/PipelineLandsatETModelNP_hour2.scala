...
import com.baitsss.model.PipelineLandsatETModelNP_hour.{DetailedMetrics, trackOperation}
...
object PipelineLandsatETModelNP_hour2 {
    ...
    try {
      ...
      val (prismKV, prismMetrics) = trackOperation("PRISM_Loading", listener) {
        RasterOperationsFocal.reshapeNN(
          sc.geoTiff[Float](maybeFolderPrism.get), _ => metadata, numParts
        ).persist(StorageLevel.MEMORY_AND_DISK_SER)
      }
      dayMetrics += prismMetrics

      // Track Landsat processing
      val (landsKV, landsatMetrics) = trackOperation("Landsat_Processing", listener) {
        ...
        val result: RDD[ITile[Array[Float]]] = RasterOperationsLocal.mapPixels(lsOv, (p: Array[Int]) => {
          try {
            if (p.length < 2) {
              Array(0.001f, 0.001f)
            } else {
      ...
      val (baseKV, baseMetrics1): (RDD[ITile[Array[Float]]], DetailedMetrics) = trackOperation("Base_Overlay_Step1", listener) {
        val result:RDD[ITile[Array[Float]]] = RasterOperationsLocal.overlay(soilAWCKV, soilFCKV, nlcdKV, elevKV, prismKV)
        val count = result.count() // Force materialization
        println(s"[MATERIALIZED] Base_Overlay_Step1: $count records")
        result
      }

      val (baseKV_full, baseMetrics2): (RDD[ITile[Array[Float]]], DetailedMetrics) = trackOperation("Base_Overlay_Step2", listener) {
        val result:RDD[ITile[Array[Float]]] = RasterOperationsLocal.overlay(baseKV, landsKV).persist(StorageLevel.MEMORY_AND_DISK_SER)
        val count = result.count() // Force materialization
      ...
      for ((path, idx) <- NLDAS_RasterRDD.zipWithIndex) {
        ...
        try {
          val (newCycle, hourMetrics) = trackOperation(s"Hour_${idx + 1}_Processing", listener) {
            ...
            // Overlay operation - specify explicit return type using overlay method
            // Overlay operation
            val (layered, overlayMetrics): (RDD[ITile[Array[Float]]], DetailedMetrics) = trackOperation(s"Hour_${idx + 1}_Overlay", listener) {
              if (cycle == null) {
                println("[DEBUG] First hour - no previous cycle")
                val result:RDD[ITile[Array[Float]]]  = RasterOperationsLocal.overlay(baseKV_full, hrKV_filter)
                val count = result.count() // Force materialization
                println(s"[MATERIALIZED] Base_Overlay_Step2: $count records")
                result
              } else {
                println("[DEBUG] Continuing from previous cycle")
                val result:RDD[ITile[Array[Float]]] = RasterOperationsLocal.overlay(baseKV_full, hrKV_filter, cycle)
                val count = result.count() // Force materialization
                println(s"[MATERIALIZED] Base_Overlay_Step2: $count records")
                result
              }
            ...
            val rawNext =
              BandCalculationCheck2.IterativeCalculationPre(layered, null)
            .persist(StorageLevel.MEMORY_AND_DISK_SER)
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Load static layers with tracking
    val (elevKV, _) = trackOperation("Load_Elevation", comprehensiveListener) {
      ...
      val result = RasterOperationsFocal.reshapeNN(elevRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
      val count = result.count() // Force materialization
    ...
    val (awcKV, _) = trackOperation("Load_AWC", comprehensiveListener) {
      ...
      val result = RasterOperationsFocal.reshapeNN(awcRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
      val count = result.count() // Force materialization
      println(s"[MATERIALIZED] NLCD: $count records")
      result
    }
    ...
    val (fcKV, _) = trackOperation("Load_FC", comprehensiveListener) {
      ...
      val result = RasterOperationsFocal.reshapeNN(fcRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
      val count = result.count() // Force materialization
      println(s"[MATERIALIZED] NLCD: $count records")
      result
    }
    ...
    val (nlcdKV, _) = trackOperation("Load_NLCD", comprehensiveListener) {
      ...
      val result = RasterOperationsFocal.reshapeNN(nlcdRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
      val count = result.count() // Force materialization
      println(s"[MATERIALIZED] NLCD: $count records")
      result
    }
    ...
    var prevOpt: Option[RDD[ITile[Array[Float]]]] = None

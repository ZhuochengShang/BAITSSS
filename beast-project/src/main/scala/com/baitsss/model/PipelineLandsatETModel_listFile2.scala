...
object PipelineLandsatETModel_listFile2 {
...
//    val Planet_filter = listDateFolders(new Path(Planet_daily), FileSystem.get(sc.hadoopConfiguration))
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      val landsat_RDD_B4_reshape: RasterRDD[Int] = RasterOperationsFocal.reshapeNN(Landsat_RDD_B4, RasterMetadata => targetMetadataGeom).persist(StorageLevel.DISK_ONLY)
      val landsat_RDD_B5_reshape: RasterRDD[Int] = RasterOperationsFocal.reshapeNN(Landsat_RDD_B5, RasterMetadata => targetMetadataGeom).persist(StorageLevel.DISK_ONLY)
...
//      val fillNAN: RasterRDD[Array[Float]] = RasterOperationsLocal.mapPixels(Landsat, (pixel: Array[Float]) => {
//              val values: Array[Float] = Array(0.0f, 0.0f)
//              if (pixel != null && pixel.length >= 1 && !pixel(0).isNaN) {
//                values(0) = pixel(0)
//                if (pixel.length >= 2) {
//                  values(1) = pixel(1)
//                }
...
//      })
      ...
      val single_band_overlay: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(SOIL_AWC_reshape, SOIL_FC_reshape, NLCD_reshape, ELEV_reshape, PRISM_reshape) //.asInstanceOf[RasterRDD[Array[Float]]]
      val singel_band_overlay_filter = single_band_overlay.filterPixels { arr =>
        arr != null && !arr.exists(_.isNaN) && arr.length == 5
      }
      ...
      NLDAS_RasterRDD.foreach(t => {
        ...
        val upsampling = RasterOperationsFocal.reshapeNN(NLDAS_obtain_values, RasterMetadata => targetMetadataGeom, singel_band_overlay_filter.getNumPartitions)//.persist(StorageLevel.DISK_ONLY)
        // (timeStamp, upsampling)
        val layer3: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(singel_band_overlay_filter, upsampling, Landsat) //.asInstanceOf[RasterRDD[Array[Float]]
        // pixel calculate
        // initial previous tile as empty tile
        val filterlayer = layer3.filterPixels { arr =>
          arr != null && !arr.exists(_.isNaN) && arr.length == 11
        }
        val res = BandCalculationCheck2.IterativeCalculationPre(filterlayer, previousCycle)
    ...
    val output_file = s"$output/landsat-$startDate-$endDate.bilinear.tif"

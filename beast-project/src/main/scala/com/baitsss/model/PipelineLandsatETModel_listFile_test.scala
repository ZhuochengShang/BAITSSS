...
object PipelineLandsatETModel_listFile_test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val matchingLandsatFolders_B4: Map[LocalDate, Seq[String]] = new LoadLandsat().findLandsatFilesForTwoMonthWindowPaths(startDate_landsat, Landsat_daily_B4, 10, sc.hadoopConfiguration)
    ...
    val NLDAS_base = "NLDAS_2023_Geotiff"


    // --- Load AOI + Landsat to define targetMetadataGeom ---
    ...
    val Landsat_RDD_Metadata = PipelineLandsatETModel_listFile_test.Raster_metadata(Landsat_metadata, Landsat_all_metadata, CRS.decode("EPSG:4326"))
    val targetMetadataGeom = PipelineLandsatETModel_listFile_test.AOI_metadata(AOI, Landsat_RDD_Metadata)

    // --- Load today's NLDAS files ---
    val today = LocalDate.parse("2023-01-01")
    val NLDAS_filter = PipelineLandsatETModel_listFile_test.listDateFolders(new Path(NLDAS_base), FileSystem.get(sc.hadoopConfiguration))
    val maybeFolderNLDAS: Option[String] = NLDAS_filter.find { case (monthDay, _) => monthDay == extractMonthDay(today) }.map(_._2)
    val NLDAS_folder = maybeFolderNLDAS.getOrElse(throw new NoSuchElementException(s"No matching NLDAS folder found"))

    val NLDAS_RasterRDD: Array[(Timestamp, String)] = FileReader.ListDate_NLDAS_hourly(NLDAS_folder).toArray

    val individualTimes = new ArrayBuffer[(String, Double)]()
    val output = "aoidata_nldas_nnlazy"
    val fs = FileSystem.get(sc.hadoopConfiguration)
    val outputPath = new Path(output)
    if (!fs.exists(outputPath)) {
      fs.mkdirs(outputPath)
    }
    // --- Reshape each hourly NLDAS ---
    var hour = 0
    for ((timestamp, path) <- NLDAS_RasterRDD) {
      println(s"Starting reshapeNN for $path")
      ...
      val reshaped = RasterOperationsFocal.reshapeNNlazy(raw, _ => targetMetadataGeom)

      val output_file = s"$output/nldas-2023-01-01.$hour.lzw.tif"
      GeoTiffWriter.saveAsGeoTiff(reshaped, output_file, Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_LZW, GeoTiffWriter.BigTiff -> "yes", GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"
      val end = System.nanoTime()
      val elapsed = (end - start) / 1e9
      individualTimes.append((path, elapsed))
      println(f"Finished reshapeNN for $path in $elapsed%.2f seconds")
      hour += 1
    }
    // --- Total Time ---
    val totalElapsed = (System.nanoTime() - startTotalTime) / 1e9
    println("------ INDIVIDUAL RESHAPE TIMES ------")
    individualTimes.foreach { case (file, time) =>
      println(f"$file: $time%.2f seconds")
    }

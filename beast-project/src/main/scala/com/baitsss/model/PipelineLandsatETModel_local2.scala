...
import com.baitsss.model.PipelineLandsatETModel_local.listDateFolders
...
object PipelineLandsatETModel_local2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val Landsat: RasterRDD[Array[Float]] = LandsatOverlay.persist(StorageLevel.DISK_ONLY)
    val numPartitions = Landsat.getNumPartitions
    ...
    val NLCD_RDD: RasterRDD[Int] = sc.geoTiff(nlcd)
    ...
    val ELEV_RDD_reshape = RasterOperationsFocal.reshapeNN(ELEV_RDD,RasterMetadata => targetMetadataGeom, numPartitions)
    val SOIL_AWC_RDD_reshape = RasterOperationsFocal.reshapeNN(SOIL_AWC_RDD,RasterMetadata => targetMetadataGeom, numPartitions)
    val SOIL_FC_RDD_reshape = RasterOperationsFocal.reshapeNN(SOIL_FC_RDD,RasterMetadata => targetMetadataGeom, numPartitions)
    val NLCD_RDD_reshape = RasterOperationsFocal.reshapeNN(NLCD_RDD_Float,RasterMetadata => targetMetadataGeom, numPartitions)
    val PRISM_reshape: RasterRDD[Float] = RasterOperationsFocal.reshapeNN(PRISM_RDD, RasterMetadata => targetMetadataGeom, numPartitions)//.persist(StorageLevel.DISK_ONLY)

    val NLDAS = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/NLDAS2019_Geotiff_test"
    val NLDAS_filter = listDateFolders(new Path(NLDAS), FileSystem.get(sc.hadoopConfiguration))

    val single_band_overlay: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(SOIL_AWC_RDD_reshape, SOIL_FC_RDD_reshape, NLCD_RDD_reshape, PRISM_reshape, ELEV_RDD_reshape) //.asInstanceOf[RasterRDD[Array[Float]]]
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      NLDAS_RasterRDD.foreach(t => {
        ...
        val NLDAS_obtain_values: RasterRDD[Array[Float]] = NLDAS_RDD_curr.mapPixels(p => {
        ...
        val upsampling: RasterRDD[Array[Float]] = RasterOperationsFocal.reshapeNN(NLDAS_obtain_values, RasterMetadata => targetMetadataGeom, numPartitions) //.persist(StorageLevel.DISK_ONLY)
        // (timeStamp, upsampling)
        val layer3: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(single_band_overlay, upsampling, Landsat) //.asInstanceOf[RasterRDD[Array[Float]]
        // pixel calculate
        // initial previous tile as empty tile
        val res = BandCalculationCheck2.IterativeCalculationPre(layer3, previousCycle)
    ...
    val output_file = s"$output/landsat-$startDate-$endDate.tif"

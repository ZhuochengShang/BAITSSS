...
import com.baitsss.model.PipelineLandsatETModel_listFile.{AOI_metadata, Raster_metadata}
...
object PipelineLandsatETModel_listFile_check {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val output = "aoidata_rdpro_landsat"
    ...
    val endDate = LocalDate.parse("2023-01-10")
    val startDate_landsat = LocalDate.parse("2021-01-01")
    val endDate_landsat = LocalDate.parse("2021-01-03")
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      println(s"Matching Landsat B5 folders: $wholePathStringB5")
      println(s"For current data: $currentDate and $currentDate_landsat")
...
//      })
      ...
      val output_file = s"$output/landsat-$currentDate-$currentDate_landsat.tif"
      GeoTiffWriter.saveAsGeoTiff(LandsatOverlay, output_file, Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_NONE, GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"

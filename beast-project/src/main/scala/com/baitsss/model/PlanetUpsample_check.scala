  ...
  def main(args: Array[String]): Unit = {
    ...
    var previousCycle: RasterRDD[Array[Float]] = null
      ...
      var hour = 0
        ...
        val output_file = s"$output/planet-$startDate-$endDate.up-$hour.tif"
        GeoTiffWriter.saveAsGeoTiff(upsampling, output_file, Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_DEFLATE, GeoTiffWriter.OutputNumBands -> "4", GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"
        hour += 1

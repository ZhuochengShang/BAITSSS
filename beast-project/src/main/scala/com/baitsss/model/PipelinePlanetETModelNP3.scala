...
import com.baitsss.model.PipelinePlanetETModel.listDateFolders
...
object PipelinePlanetETModelNP3 {
  ...
  def processDay(
    ...
    try {
      ...
      val prismKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(
        sc.geoTiff[Float](maybeFolderPrism.get), _ => metadata, numParts, part)//.persist(StorageLevel.MEMORY_AND_DISK_SER)
      ...
      // Create a thread-safe map to store timestamp -> output path
      val results = new java.util.concurrent.ConcurrentHashMap[Timestamp, String]()

      // Parallel processing of NLDAS files
      Parallel2.forEach(NLDAS_RasterRDD.length, (startIdx, endIdx) => {
        for (i <- startIdx until endIdx) {
          ...
          println(s"[INFO] Reshaping NLDAS file: $fileName")
          ...
          try {
            ...
            val hrKV: RDD[(Int, ITile[Array[Float]])] = RasterOperationsFocal.reshapeNN2(rawIn, _ => metadata, numParts, part)
            ...
            val outputPath = s"workflow-data/${timestamp.getTime}_$fileName"
            hrKV_filter.saveAsObjectFile(outputPath)

            // Store result in the map
            results.put(timestamp, outputPath)

            println(s"[INFO] Finish Reshaping NLDAS file: $fileName")
            val totalElapsed = (System.nanoTime() - startTotalTime) / 1e9
            println(f"[INFO] Processed $fileName in $totalElapsed%.2f seconds")
          } catch {
            case e: Exception =>
              println(s"[ERROR] Failed to process $fileName: ${e.getMessage}")
              e.printStackTrace()
          }
        }
      })

      // Get sorted timestamps for sequential processing
      val sortedTimestamps = new java.util.ArrayList(results.keySet())
      java.util.Collections.sort(sortedTimestamps)

      // Process files in timestamp order
      var idx = 0
      val fs = FileSystem.get(sc.hadoopConfiguration)
      var cycle: RDD[(Int, ITile[Array[Float]])] = prevOpt.orNull
      for (i <- 0 until sortedTimestamps.size()) {
        val timestamp = sortedTimestamps.get(i)
        val outputPath = results.get(timestamp)

        println(s"\n=== Hour ${idx + 1} Processing ===")
        println(s"[DEBUG] Processing timestamp: $timestamp")
...
//            p(5) // Shortwave
//          ))
          val hrKV_filter = sc.objectFile[(Int, ITile[Array[Float]])](outputPath).partitionBy(part)
          // Overlay operation
          ...
          cycle = rawNext
          // Clean up the saved objectFile after processing
//          try {
//            println(s"[INFO] Cleaning up temporary file: $outputPath")
//            fs.delete(new Path(outputPath), true) // true means recursive delete
//          } catch {
//            case e: Exception =>
//              println(s"[WARN] Failed to delete temporary file $outputPath: ${e.getMessage}")
//          }
          idx += 1
  ...
  def main(args: Array[String]): Unit = {
    ...
    val Planet_daily = "Planet_CA_4B_BAITSSS"
    val sample: RasterRDD[Array[Int]] = sc.geoTiff("Planet_CA_4B_BAITSSS/2023-01-01")

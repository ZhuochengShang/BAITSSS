...
object PipelineLandsatETModelNP4 {
  ...
  def processDay(
    ...
    try {
      ...
      val prismKV: RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(
        prismRetile, _ => metadata, numParts).persist(StorageLevel.MEMORY_AND_DISK_SER)
      ...
      val baseKV: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(soilAWCKV, soilFCKV, nlcdKV, elevKV, prismKV).persist(StorageLevel.MEMORY_AND_DISK_SER)
      val materize: RDD[ITile[Array[Float]]] =
        RasterOperationsLocal
          .overlay(baseKV, landsKV) //materize.persist(StorageLevel.MEMORY_AND_DISK_SER)
      val baseKV_full = materize.persist(StorageLevel.MEMORY_AND_DISK_SER) //new ShuffledRDD[Int, ITile[Array[Float]], ITile[Array[Float]]](RasterOperationsLocal.mapPixelsPreservePartitionerKeyed(materize, (temp: Array[Float]) => {
//        temp
//      }), part).persist(StorageLevel.MEMORY_AND_DISK_SER)
      ...
      for ((path, idx) <- NLDAS_RasterRDD.zipWithIndex) {
        ...
        try {
          ...
          val hrKV: RDD[ITile[Array[Float]]] = RasterOperationsFocal.reshapeNN(hrRDD_part, _ => metadata, numParts)
          // Overlay operation
          val layered: RasterRDD[Array[Float]] = if (cycle == null) {
            println("[DEBUG] First hour - no previous cycle")
            RasterOperationsLocal.overlay(baseKV_full, hrKV)
          } else {
            println("[DEBUG] Continuing from previous cycle")
            RasterOperationsLocal.overlay(baseKV_full, hrKV, cycle)
          }
          ...
          //val rawNext = new ShuffledRDD[Int, ITile[Array[Float]], ITile[Array[Float]]](BandCalculationCheck2.IterativeCalculationPre3(layered, null), part).persist(StorageLevel.MEMORY_AND_DISK_SER)
          val rawNext = BandCalculationCheck2.IterativeCalculationPre(layered, null).persist(StorageLevel.MEMORY_AND_DISK_SER)
          if (rawNext == null) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val elevKV: RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(elevRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val awcKV: RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(awcRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val fcKV: RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(fcRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val nlcdKV: RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(nlcdRDD, _ => aoiMeta, parts).persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val output = "aoidata_rdpro_landsat_rs"

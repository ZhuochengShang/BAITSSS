...
import org.apache.spark.{HashPartitioner, Partitioner, ShuffleDependency, SparkConf}
...
//  }
  ...
  def enforcePartitionKV[T: ClassTag](
                                       rdd: RasterRDD[T], part: Partitioner, sort: Boolean = false
                                     ): RDD[(Int, ITile[T])] = {
    val kv = rdd.map(tile => (tile.tileID, tile)).partitionBy(part)
    if (sort) kv.mapPartitions(_.toArray.sortBy(_._1).iterator, preservesPartitioning = true)
    else kv
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val wholePathStringB4 = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/landsat_test"
    ...
    val landsKV = enforcePartitionKV(
      Landsat, part, sort = true)//.filter(_._2 != null)

//
    val NLDAS = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/aoidata_rdpro/nldas-2023-01-01.lzw.tif/NLDAS_FORA0125_H.A20230101.0000.002.tif.lzw.tif/NLDAS_FORA0125_H.A20230101.0000.002.002.tif"
    ...
    val hrTiles: RDD[ITile[Array[Float]]] =
      RasterOperationsFocal
        .reshapeNN(NLDAS_RDD_curr, _ => targetMetadataGeom, targetNumPartition)
    val hrKV: RDD[(Int, ITile[Array[Float]])] =
      enforcePartitionKV(
        hrTiles, part, sort = true)

    val layered = RasterOperationsLocal.overlayArrayStreamsF2(hrKV, landsKV)
    val next5: RDD[ITile[Array[Float]]] =
      RasterOperationsLocal.mapPixels(
        layered.values,
        (p: Array[Float]) => Array(
    ...
    GeoTiffWriter.saveAsGeoTiff(next5, output_file+"/lazyCA_overlay.tif", Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.OutputNumBands -> "5", GeoTiffWriter.BigTiff -> "yes", GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"

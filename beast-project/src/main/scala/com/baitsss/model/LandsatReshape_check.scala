...
object LandsatReshape_check {
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      val output_file = s"$output/landsat-$currentDate-$currentDate_landsat.reshape.tif"
      GeoTiffWriter.saveAsGeoTiff(LandsatOverlay, output_file, Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"

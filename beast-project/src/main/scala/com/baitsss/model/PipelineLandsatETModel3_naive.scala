...
object PipelineLandsatETModel3_naive {
  ...
  def processDay(
                  ...
                  numParts: Int,
                  part: Partitioner,
                  prevOpt: Option[RDD[ITile[Array[Float]]]],
                  soilAWCKV: RDD[ITile[Float]],
                  soilFCKV: RDD[ITile[Float]],
                  nlcdKV: RDD[ITile[Float]],
                  elevKV: RDD[ITile[Float]],
                  stageCounter: Option[StageCountingListener] = None
                ): RDD[ITile[Array[Float]]] = {
    ...
    try {
      ...
      // Load PRISM data
      val prism_daily = "file:///Users/clockorangezoe/Desktop/BAITSSS_project/data/PRISM_2023_Geotiff/2023-08-10"
      ...
      val prismKV:RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(
        sc.geoTiff[Float](prism_daily), _ => metadata, numParts)
...
//      Thread.sleep(300000) // 5 minutes
      ...
      val b4Map = findLocalLandsatFiles(
        landsatDate,
        "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/centralvalley/landsat/B4",
        7
      )
      val b5Map = findLocalLandsatFiles(
        landsatDate,
        "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/centralvalley/landsat/B5",
        7
      )
      ...
      val rs4: RDD[ITile[Int]] = RasterOperationsFocal.reshapeNN(b4RDD, _ => metadata, numParts)
      val rs5: RDD[ITile[Int]] = RasterOperationsFocal.reshapeNN(b5RDD, _ => metadata, numParts)
      val lsOv: RDD[ITile[Array[Int]]] = RasterOperationsLocal.overlay(rs4, rs5)
      val landsKV: RDD[ITile[Array[Float]]] = RasterOperationsLocal.mapPixels(lsOv, (p:Array[Int]) => {
        try {
          if (p.length < 2) {
            Array(0.0f, 0.0f)
          } else {
      ...
      val baseKV: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(soilAWCKV, soilFCKV, nlcdKV, elevKV, prismKV)
      val baseKV_full: RDD[ITile[Array[Float]]] =
        RasterOperationsLocal
          .overlay(baseKV, landsKV)

      // Process hourly data
      val dayDir = new File(s"/Users/clockorangezoe/Desktop/BAITSSS_project/data/NLDAS_2023_Geotiff/$dateStr")
      ...
      val tifPathcheck = tifPaths.slice(0,2)

      val output = "file:///Users/clockorangezoe/Desktop/BAITSSS_project"

      var cycle: RDD[ITile[Array[Float]]] = prevOpt.orNull
      println(s"[DEBUG] Initial cycle is null: ${cycle == null}")

      for ((path, idx) <- tifPathcheck.zipWithIndex) {
        ...
        try {
          ...
          val hrKV: RDD[ITile[Array[Float]]] = RasterOperationsFocal.reshapeNN(hrRDD, _ => metadata, numParts)
          //val hrKV: RDD[(Int, ITile[Array[Float]])] = enforcePartitionKV(RasterOperationsFocal.reshapeBilinear(hrRDD, _ => metadata, numParts),part)
          val hrKV_filter: RDD[ITile[Array[Float]]] = RasterOperationsLocal.mapPixels(hrKV, (p: Array[Float]) => Array(
            p(0), // Air Temp
          ...
          // Overlay operation
          val layered: RasterRDD[Array[Float]] = if (cycle == null) {
            println("[DEBUG] First hour - no previous cycle")
            RasterOperationsLocal.overlay(baseKV_full, hrKV_filter)
          } else {
            println("[DEBUG] Continuing from previous cycle")
            RasterOperationsLocal.overlay(baseKV_full, hrKV_filter, cycle)
          }

          // Calculate next iteration
          val rawNext = BandCalculationCheck2.IterativeCalculationPre(layered, null)
          if (rawNext == null) {
...
//          Thread.sleep(300000) // 5 minutes
  ...
  def main(args: Array[String]): Unit = {
    ...
    val aoi = sc.shapefile("file:///Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/centralvalley/aoi/POLYGON.shp") // Update this path
    val startDate = LocalDate.parse("2023-08-10")
    val b4SampleMap = findLandsatFilesForTwoMonthWindowPaths(
      startDate,
      "file:///Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/centralvalley/landsat/B4",
      thresholdDays = 7,
    ...
    val elevKV:RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(elevRDD, _ => aoiMeta, parts).persist(StorageLevel.DISK_ONLY)
    //val elevKV:RDD[(Int, ITile[Float])] = enforcePartitionKV(RasterOperationsFocal.reshapeBilinear(elevRDD, _ => aoiMeta, parts),part).persist(StorageLevel.DISK_ONLY)
    val awcKV:RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(awcRDD, _ => aoiMeta, parts).persist(StorageLevel.DISK_ONLY)
    val fcKV:RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(fcRDD, _ => aoiMeta, parts).persist(StorageLevel.DISK_ONLY)
    val nlcdKV:RDD[ITile[Float]] = RasterOperationsFocal.reshapeNN(nlcdRDD, _ => aoiMeta, parts).persist(StorageLevel.DISK_ONLY)
...
//    Thread.sleep(300000) // 5 minutes
    ...
    var prevOpt: Option[RDD[ITile[Array[Float]]]]
    = None
    ...
    val finalRdd: RasterRDD[Array[Float]] = prevOpt.get.mapPixels(arr => {
      val out = Array.fill(5)(-9999f)
      Array.copy(arr, 0, out, 0, math.min(arr.length, 5))
      out
    })
    val output = "file:///Users/clockorangezoe/Desktop/BAITSSS_project"
    val output_file = s"$output/landsat-$star-$endDate.ca.nn.tif"

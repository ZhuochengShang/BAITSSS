...
object PipelineLandsatETModelNP_checkp {
...
// 51 seconds for base full eager
      ...
      for ((path, idx) <- NLDAS_RasterRDD.zipWithIndex) {
        ...
        try {
          ...
          val dateinstr = datein.toString
          val checkpointDir = "/local_data/dblab/chkpt/${dateinstr}_$idx"
          sc.setCheckpointDir(checkpointDir)
          val rawNext = BandCalculationCheck2.IterativeCalculationPre3(layered, null)//.persist(StorageLevel.MEMORY_AND_DISK_SER)
          rawNext.checkpoint() // <-- Add this line to trigger checkpointing
          rawNext.count() // Trigger checkpoint
          ...
          cycle = enforcePartitionKV(rawNext.values,part) //.persist(StorageLevel.MEMORY_AND_DISK_SER)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val checkpointDir = "file:///local_data/downloads/chkpt"
//    sc.setCheckpointDir(checkpointDir)
    //spark.sparkContext.setCheckpointDir("/local_data/downloads/chkpt_new")
    //sc.setLogLevel("WARN")
    ...
    val awcRDD_filled = awcRDD.filterPixels(_ > 0)
    val fcRDD = sc.geoTiff[Float]("Soil_2023/fc_gNATSGO_US.tif")
    val fcRDD_filled = fcRDD.filterPixels(_ > 0)
...
//    val nlcdKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(nlcdRDD, _ => aoiMeta, parts, part).persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val elevKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(elevRDD, _ => aoiMeta, parts, part).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val awcKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(awcRDD_filled, _ => aoiMeta, parts, part).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val fcKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(fcRDD_filled, _ => aoiMeta, parts, part).persist(StorageLevel.MEMORY_AND_DISK_SER)
    val nlcdKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeNN2(nlcdRDD, _ => aoiMeta, parts, part).persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val end = LocalDate.parse("2023-01-30")
    ...
    while (!curr.isAfter(end)) {
      ...
      if (daysSinceStart % 31 == 0) {
        ...
        val intermediateOutput = s"intermediate_results/day_${daysSinceStart}_${curr.toString}.tif"
        GeoTiffWriter.saveAsGeoTiff(rasterForWrite, intermediateOutput, Seq(
        ...
        println(s"[GEOTIFF WRITTEN] $intermediateOutput")

        // Clean up current RDDs
        if (prevOpt.isDefined) prevOpt.get.unpersist(blocking = true)
        daily.unpersist(blocking = true)
        rasterForWrite.unpersist()

        // **KEY PART**: Read back and convert to the EXACT same format as prevOpt
        val reloadedRaster: RasterRDD[Array[Float]] = sc.geoTiff[Array[Float]](intermediateOutput)

        // Convert back to keyed format that prevOpt expects: RDD[(Int, ITile[Array[Float]])]
        val reloadedKV: RDD[(Int, ITile[Array[Float]])] =
          enforcePartitionKV(reloadedRaster,part)
            .persist(StorageLevel.MEMORY_AND_DISK_SER)

        //reloadedKV.count() // Force evaluation

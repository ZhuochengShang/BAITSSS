...
object PipelineLandsatETModel_local {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // AOI
    val AOI_shp = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/yuma/YumaAgMain_latlon3.shp"
    ...
    val lai = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/lai_toa.img"
    val ndvi = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/ndvi_toa.img"
    val precip_prism = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/PRISM_tmin_stable_4kmD2_20190923_bil.tif"
    ...
    val ELEV_RDD: RasterRDD[Float] = sc.geoTiff(elev)
    val SOIL_AWC_RDD: RasterRDD[Float] = sc.geoTiff(soil_awc).persist(StorageLevel.DISK_ONLY)
    val SOIL_FC_RDD: RasterRDD[Float] = sc.geoTiff(soil_fc).persist(StorageLevel.DISK_ONLY)
    val NLCD_RDD: RasterRDD[Float] = sc.geoTiff(nlcd)
    val NLCD_RDD_Float = NLCD_RDD.mapPixels(_.toFloat).persist(StorageLevel.DISK_ONLY)

    val NLDAS = "/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/originaldata/NLDAS2019_Geotiff_test0"
    val NLDAS_filter = listDateFolders(new Path(NLDAS), FileSystem.get(sc.hadoopConfiguration))

       // Define start and end dates.
    val startDate = LocalDate.parse("2019-09-23")
    val endDate = LocalDate.parse("2019-09-23")
    val startDate_landsat = LocalDate.parse("2019-09-23")
    val endDate_landsat = LocalDate.parse("2019-09-23")
    ...
    while (!currentDate.isAfter(endDate)) {
      ...
      val windowDays: Long = 16L

      // Calculate end date string
      val endDateStr: String = extractMonthDay(currentDate.plusDays(windowDays)) // e.g., "01-17"

//      val Landsat_RDD_B4: RasterRDD[Int] = sc.geoTiff(Landsat_daily_B4)
//      val Landsat_RDD_B5: RasterRDD[Int] = sc.geoTiff(Landsat_daily_B5)

      val ndvi_toa: RasterRDD[Float] = sc.geoTiff("/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/aoidata_reshape/ndvi_toa.img.filled.tif")
      val lai_toa: RasterRDD[Float] = sc.geoTiff("/Users/clockorangezoe/Desktop/BAITSSS_project/testfolder/aoidata_reshape/lai_toa.img.filled.tif")

      // todo: read - write back see value changes

      val Landsat_metadata = ndvi_toa.first().rasterMetadata
      val Landsat_all_metadata = RasterMetadata.allMetadata(ndvi_toa)
      ...
      val landsat_RDD_B4_reshape: RasterRDD[Float] = RasterOperationsFocal.reshapeNN(ndvi_toa, RasterMetadata => targetMetadataGeom).persist(StorageLevel.DISK_ONLY)
      val landsat_RDD_B5_reshape: RasterRDD[Float] = RasterOperationsFocal.reshapeNN(lai_toa, RasterMetadata => targetMetadataGeom).persist(StorageLevel.DISK_ONLY)

      val LandsatOverlay: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(landsat_RDD_B4_reshape, landsat_RDD_B5_reshape)
      val Landsat: RasterRDD[Array[Float]] = LandsatOverlay
...
//          pixelValue = -1
//        }
//        val LAI = new CalculationLAI().ndviLaiFunc(pixelValue)
...
//      })//.persist(StorageLevel.DISK_ONLY)
      ...
      val PRISM_RDD: RasterRDD[Float] = sc.geoTiff(precip_prism)
      ...
      val single_band_overlay: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(SOIL_AWC_reshape, SOIL_FC_reshape, NLCD_reshape, PRISM_reshape, ELEV_reshape) //.asInstanceOf[RasterRDD[Array[Float]]]
      //single_band_overlay.persist(StorageLevel.DISK_ONLY)
      val output_file = s"$output/landsat-$startDate-$endDate-0.tif"
      GeoTiffWriter.saveAsGeoTiff(single_band_overlay, output_file + "_single_band.tif", Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_DEFLATE, GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"
      ...
      NLDAS_RasterRDD.foreach(t => {
        ...
        val layernldas: RasterRDD[Array[Float]] = sc.geoTiff("/Users/clockorangezoe/Documents/phd_projects/code/pythonProject/stacked_output.tif") //.asInstanceOf[RasterRDD[Array[Float]])
        val NLDAS_obtain_values: RasterRDD[Array[Float]] = layernldas.mapPixels( p => {
          val values = new Array[Float](4)
          values(0) = p(5) // Tair_oC
          values(1) = p(6) // S_hum
          values(2) = p(7)
          values(3) = p(8) // In_short
          values
        })

//          NLDAS_RDD_curr.mapPixels(p => {
//          val values = new Array[Float](4)
...
//        })
        ...
        val upsampling: RasterRDD[Array[Float]] = RasterOperationsFocal.reshapeNN(NLDAS_obtain_values, RasterMetadata => targetMetadataGeom, single_band_overlay.getNumPartitions)//.persist(StorageLevel.DISK_ONLY)
        // (timeStamp, upsampling)
        var layer3: RasterRDD[Array[Float]] = RasterOperationsLocal.overlay(single_band_overlay, upsampling, Landsat) //.asInstanceOf[RasterRDD[Array[Float]]
        GeoTiffWriter.saveAsGeoTiff(layer3, output_file + "_stack_all.tif", Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_DEFLATE, GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"

        if (previousCycle != null && !previousCycle.isEmpty())
            layer3 = RasterOperationsLocal.overlay(layer3, previousCycle)
        // pixel calculate
        // initial previous tile as empty tile
        previousCycle = BandCalculationCheck2.IterativeCalculationPre(layer3, previousCycle)
        println("Finish hour: ", hour)
        hour += 1
        upsampling.unpersist()
      })
      // Increment to the next day.
      currentDate = currentDate.plusDays(1)
    }
    val output_file = s"$output/landsat-$startDate-$endDate-0.tif"
    val ET_sum: RasterRDD[Float] = previousCycle.mapPixels(x => x(0))
    GeoTiffWriter.saveAsGeoTiff(ET_sum, output_file + "_et_sum.tif", Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_DEFLATE, GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"
    GeoTiffWriter.saveAsGeoTiff(previousCycle, output_file, Seq(GeoTiffWriter.WriteMode -> "compatibility", GeoTiffWriter.Compression -> TiffConstants.COMPRESSION_DEFLATE, GeoTiffWriter.FillValue -> "-9999")) //GeoTiffWriter.BigTiff -> "yes"

...
import com.baitsss.model.PipelinePlanetETModelNP3.{AOI_metadata, Raster_metadata}
...
object PipelinePlanetETModelNP {
  ...
  def processDay(
    ...
    try {
      ...
      val Planet_daily = "sample/centralvalley/PSScene"
...
//      println(s"Found Planet folder: $planetFolder")
      ...
      for ((path, idx) <- NLDAS_RasterRDD.zipWithIndex) {
        ...
        try {
          ...
          val hrKV: RDD[(Int, ITile[Array[Float]])] = RasterOperationsFocal.reshapeBilinearSortPartition(hrRDD_part, _ => metadata, numParts, part)
          // Overlay operation
  ...
  def main(args: Array[String]): Unit = {
    ...
    val aoi = sc.shapefile("sample/aoi/aoi.shp")
    ...
    val sample: RasterRDD[Array[Int]] = sc.geoTiff("sample/centralvalley/PSScene")

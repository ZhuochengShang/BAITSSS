...
object PipelinePlanetETModelNP_sp {
  ...
  def processDay(
    ...
    try {
      ...
      val prismKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeBilinearSortPartition(
        prismRetile, _ => metadata, numParts, part).persist(StorageLevel.DISK_ONLY)
      ...
      val Planet_daily = "sample/composite4326_2.tif"

      // 1) Find today's Planet folder
      // 2) Load & NDVI→LAI
      val Planet_RDD: RasterRDD[Array[Float]] = sc.geoTiff(Planet_daily)
      val retile = Planet_RDD.retile(256,256)
      val lsOv = enforcePartitionKV(retile, part)
      ...
      for ((path, idx) <- NLDAS_RasterRDD.zipWithIndex) {
        ...
        try {
          ...
          val output_file = s"$output/planet-$datein-$idx.nn.tif"
          val finalRdd: RasterRDD[Float] = cycle.values.mapPixels(arr => {
            arr(0)
          })
  ...
  def main(args: Array[String]): Unit = {
    ...
    val Planet_daily = "sample/2023-08-22"
    val sample: RasterRDD[Array[Float]] = sc.geoTiff("sample/composite4326_2.tif")
    val retile = sample.retile(256,256)
    val initial = retile.first().rasterMetadata
    val aoiMeta = initial
    ...
    val elevKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeBilinearSortPartition(elevRDD, _ => aoiMeta, parts, part).persist(StorageLevel.DISK_ONLY)
    val awcKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeBilinearSortPartition(awcRDD, _ => aoiMeta, parts, part).persist(StorageLevel.DISK_ONLY)
    val fcKV: RDD[(Int, ITile[Float])] = RasterOperationsFocal.reshapeBilinearSortPartition(fcRDD, _ => aoiMeta, parts, part).persist(StorageLevel.DISK_ONLY)
